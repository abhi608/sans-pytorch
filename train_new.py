...
from misc.lstm_models import Seq2SeqFastAttention
...
def main(params):
    ...
    question_model = Seq2SeqFastAttention(params['emb_size'], params['emb_size'], vocab_size,
                                            vocab_size, params['hidden_size'], params['hidden_size'],
                                            params['batch_size'], 1, 1, nlayers=params['rnn_layers'],
                                            bidirectional=True, nlayers_trg=params['rnn_layers'],
                                            dropout=params['dropout'], use_gpu=params['use_gpu'])
    ...
    for epoch in range(params['resume_from_epoch'], params['epochs']+1):
        ...
        for i, (image, question, ques_len, ans) in enumerate(train_loader):
            ...
            ques_emb = question_model(Variable(question.data.long()), Variable(question.data.long()), ques_len)
            print("ques_emb after lstm: ", ques_emb.__class__, ques_emb.size())
