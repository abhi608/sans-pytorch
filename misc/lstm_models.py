...
class Seq2SeqFastAttention(nn.Module):
	...
	print(h0_encoder.size(), h0_encoder.__class__, c0_encoder.size(), c0_encoder.__class__)
        return Variable(h0_encoder.data.cuda()), Variable(c0_encoder.data.cuda())
	#return h0_encoder.cuda(), c0_encoder.cuda()

    def forward(self, input_src, input_trg, ques_len, trg_mask=None, ctx_mask=None):
        """Propogate input through the network."""
	B, W = input_src.size()
	print("input_src", input_src.size())
        src_emb = self.src_embedding(input_src)
        trg_emb = self.trg_embedding(input_trg)
	...
	x = torch.LongTensor(ques_len - 1)
        ...
        # bsize x seq_len_trg x (2 is* dim)
	tmp = torch.cat((trg_h, alpha), 1)
	print("tmp: ", tmp.size())
        trg_h_reshape = torch.cat((trg_h, alpha), 2)
	print("trg_h_reshape: ", trg_h_reshape.size())
